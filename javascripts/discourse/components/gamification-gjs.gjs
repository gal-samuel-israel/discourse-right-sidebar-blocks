...
import inc from "../helpers/inc.js";
import AlgoBadgeGjs from "./algo-badge-gjs.gjs";
...
export default class GamificationGjs extends Component {
  @tracked page = 1;
  ...
  constructor() {
    ...
    this.component_debug = settings?.enable_debug_for_gamification_component;
    ...
    const leaderboardURL = this.isAlgoSecUser
      ? `/leaderboard/4?period=${this.period}`
      : `/leaderboard/?period=${this.period}`;
    const secondboardURL = this.isAlgoSecUser
      ? `/leaderboard/?period=${this.period_2}`
      : false;

    ajax(leaderboardURL).then((scores) => {
      this.gamificatinObj = scores;
      this.gamificatinObj.users = scores.users.slice(0, this.maxUsersToShow);
    });

    if (secondboardURL !== false) {
      ajax(secondboardURL).then((scores) => {
        this.gamificatinObj_2 = scores;
        this.gamificatinObj_2.users = scores.users.slice(
          0,
          this.maxUsersToShow
        );
      });
  ...
  scoreForMonth_2(event) {
    event?.preventDefault();
    this.activeMonthly_2 = true;
    this.activeAll_2 = false;
  ...
  scoreForAll_2(event) {
    event?.preventDefault();
    this.activeMonthly_2 = false;
    this.activeAll_2 = true;
    this._changePeriod_2("all");
  }

  get headingLabel() {
    return i18n(themePrefix("gamification.heading"));
  }

  get viewAllLabel() {
    return i18n(themePrefix("gamification.view_all_tag"));
  }

  get viewAllLink() {
    return i18n(themePrefix("gamification.view_all_link"));
  }

  get viewAllLinkAlgoSecOnly() {
    return i18n(themePrefix("gamification.view_all_link_algosec_only"));
  }

  get viewAllBadgesLabel() {
    return i18n(themePrefix("gamification.view_all_badges_tag"));
  }

  get viewAllBadgesLink() {
    return i18n(themePrefix("gamification.view_all_badges_link"));
  }

  get primaryViewAllLink() {
    return this.isAlgoSecUser ? this.viewAllLinkAlgoSecOnly : this.viewAllLink;
  }
  ...
  <template>
    ...
    <div class="gamification-controls-wrap gjs-added">
      <a
        ...
        class={{this.monthlyControlClass}}
      >
        Last 30 days
      </a>
      <a
        href="/leaderboard"
        {{on "click" this.scoreForAll}}
        class={{this.allTimeControlClass}}
      >
        All time
      </a>
    </div>
          ...
          <div class="gamification--user gjs-added">
            ...
            <span class="gamification--user-score gjs-added">
              ...
              >
                ...
                />
                <path
                  d="M15.5903 13.0317C15.1174 12.4206 14.9165 11.7895 14.9961 11.0562C15.0774 10.3111 14.6442 9.60855 13.944 9.34041C13.2953 9.09191 12.8499 8.69207 12.5894 8.06981C12.2933 7.36121 11.5488 6.94851 10.7913 7.07953C10.1047 7.19836 9.51835 7.07242 8.97603 6.64347C8.34665 6.14545 7.45373 6.16949 6.85416 6.70238C6.34571 7.15401 5.81422 7.34326 5.18586 7.25084C4.38677 7.13336 3.62393 7.60564 3.3719 8.37281C3.18526 8.94159 2.84143 9.31129 2.29098 9.50732C1.53119 9.77782 1.07422 10.5656 1.22767 11.3572C1.34556 11.9642 1.20159 12.4738 0.778504 12.9467C0.230083 13.5602 0.225341 14.4899 0.76631 15.1098C1.18432 15.5881 1.3171 16.1071 1.1938 16.7237C1.03561 17.5159 1.48004 18.2952 2.23712 18.5786C2.79604 18.7878 3.14359 19.1762 3.33837 19.7632C3.58937 20.5199 4.33833 20.9888 5.12895 20.8835C5.78238 20.7962 6.33724 20.9976 6.87245 21.4604C7.46762 21.9757 8.33581 21.9977 8.95909 21.5166C9.53021 21.0755 10.143 20.94 10.8567 21.0511C11.6013 21.1669 12.3221 20.7613 12.6161 20.0679C12.8898 19.4216 13.3458 18.9991 14.0107 18.734C14.6906 18.4625 15.1099 17.7681 15.0273 17.0412C14.9419 16.292 15.1289 15.6464 15.6004 15.0251C16.0483 14.435 16.0439 13.6174 15.5903 13.0317ZM12.2713 18.195C11.1704 19.2957 9.70601 19.9017 8.14848 19.9017C6.59096 19.9017 5.12692 19.2957 4.02567 18.195C2.92443 17.0944 2.31774 15.6308 2.31774 14.0744C2.31774 12.5181 2.92443 11.0549 4.02567 9.95387C5.12658 8.85323 6.59096 8.24721 8.14848 8.24721C9.70601 8.24721 11.1701 8.85323 12.2713 9.95387C13.3725 11.0545 13.9792 12.5181 13.9792 14.0744C13.9792 15.6308 13.3725 17.0944 12.2713 18.195Z"
                  fill="#000000"
                />
                <path
                  d="M12.7625 1.03985V6.94394C12.2233 6.39447 11.4364 6.12023 10.6315 6.25837C10.1143 6.34808 9.76096 6.24313 9.42459 5.97703C8.50864 5.25251 7.19805 5.27926 6.31495 6.06437C5.7059 6.60505 5.37698 6.4009 4.91392 6.4009C4.38278 6.4009 3.88347 6.57288 3.47766 6.87251V1.03985C3.47766 0.510689 3.90854 0.0803833 4.43765 0.0803833H11.8022C12.3317 0.0803833 12.7622 0.511027 12.7622 1.03985H12.7625Z"
                  fill="#000000"
                />
              </svg>
              {{item.total_score}}
    ...
    <a href={{this.primaryViewAllLink}} class={{this.primaryViewAllClass}}>
      {{this.viewAllLabel}} {{icon "angle-right"}}
    </a>

    <a href={{this.viewAllBadgesLink}} class={{this.badgesViewAllClass}}>
      {{this.viewAllBadgesLabel}}
    </a>
    ...
    {{#if this.isAlgoSecUser}}
      ...
      <div class="gamification-controls-wrap gjs-added">
        <a
          ...
          class={{this.monthlyControlClass2}}
        >
          Last 30 days
        </a>
        <a
          href="/leaderboard"
          {{on "click" this.scoreForAll_2}}
          class={{this.allTimeControlClass2}}
        >
          All time
        </a>
      </div>
      ...
      <a href={{this.viewAllLink}} class={{this.secondaryViewAllClass}}>
        {{this.viewAllLabel}} {{icon "angle-right"}}
      </a>
    {{/if}}
  </template>
